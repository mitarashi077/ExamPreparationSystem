...
export const addToReviewList = async (req: Request, res: Response) => {
  try {
    const { questionId, isCorrect, timeSpent, deviceType } =
      reviewAnswerSchema.parse(req.body)
    ...
    if (existingReviewItem) {
      ...
      const correctStreak = isCorrect ? existingReviewItem.correctStreak + 1 : 0
...
export const getReviewSchedule = async (_req: Request, res: Response) => {
  try {
    ...
    const [todayItems, tomorrowItems, thisWeekItems, totalActiveItems] =
      await Promise.all([
        // 今日復習すべき問題
    ...
    res.json({
      ...
      masteryDistribution: masteryStats.reduce(
        (acc: any, item: any) => {
...
export const getReviewStats = async (req: Request, res: Response) => {
  try {
    ...
    Object.keys(categoryStatsMap).forEach((key) => {
      ...
      stat.avgMasteryLevel =
        Math.round((stat.totalMasteryLevel / stat.total) * 100) / 100
      stat.avgReviewCount =
        Math.round((stat.avgReviewCount / stat.total) * 100) / 100
