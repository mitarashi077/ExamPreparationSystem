// Standalone unit test for spaced repetition algorithm logic
...
describe('Spaced Repetition Algorithm - Unit Tests', () => {
  describe('getNextReviewInterval', () => {
    ...
    it('should handle invalid mastery levels gracefully', () => {
      // -1 + 1 = 0 -> Min(5, 0) = 0 -> interval[0] = 1
      // 10 + 1 = 11 -> Min(5, 11) = 5 -> interval[5] = 4320
      expect(SpacedRepetitionAlgorithm.getNextReviewInterval(-1, true)).toBe(1)
      expect(SpacedRepetitionAlgorithm.getNextReviewInterval(10, true)).toBe(
        4320,
      )
    })
  })
  ...
  describe('calculatePriority', () => {
    ...
    it('should increase priority with more wrong answers', () => {
      const lowWrong = SpacedRepetitionAlgorithm.calculatePriority(4, 1, 0) // priority = 1 + (5-4) + 0.5 + 0 = 2.5 -> 3
      const highWrong = SpacedRepetitionAlgorithm.calculatePriority(4, 2, 0) // priority = 1 + (5-4) + 1 + 0 = 3 -> 3

      expect(highWrong).toBeGreaterThanOrEqual(lowWrong)
    })

    it('should increase priority with more days since last review', () => {
      const recent = SpacedRepetitionAlgorithm.calculatePriority(3, 1, 1) // priority = 1 + 2 + 0.5 + 0.1 = 3.6 -> 4
      const old = SpacedRepetitionAlgorithm.calculatePriority(3, 1, 5) // priority = 1 + 2 + 0.5 + 0.5 = 4

      expect(old).toBeGreaterThanOrEqual(recent) // They might be equal due to rounding
    })
  ...
  describe('Integration Scenarios', () => {
    ...
    it('should calculate realistic priorities for different scenarios', () => {
      // New difficult question (just wrong) - priority = 1 + 5 + 0.5 + 0 = 6.5 -> 5 (capped)
      const newDifficult = SpacedRepetitionAlgorithm.calculatePriority(0, 1, 0)

      // Repeatedly wrong question - priority = 1 + 5 + 1.5 + 0 = 7.5 -> 5 (capped)
      ...
      // Old question not reviewed in a while - priority = 1 + 3 + 0.5 + 1.5 = 6 -> 5 (capped)
      const oldQuestion = SpacedRepetitionAlgorithm.calculatePriority(2, 1, 15)

      // Well-mastered question - priority = 1 + 1 + 0.5 + 0.1 = 2.6 -> 3
      ...
      // All difficult questions will be capped at 5, so they should be equal
      expect(repeatedlyWrong).toBe(5)
      expect(newDifficult).toBe(5)
      expect(oldQuestion).toBe(5)
      expect(masteredQuestion).toBe(3)
      expect(masteredQuestion).toBeLessThan(newDifficult)
    })
