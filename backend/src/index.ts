    ...
    const sampleQuestions = await Promise.all([
      ...
      prisma.question.create({
        data: {
          ...
          session: '春期',
          categoryId:
            categories.length > 2 ? categories[2].id : categories[0].id,
          choices: {
            create: [
        ...
        data: {
          ...
          choices: {
            create: [
              ...
              {
                content:
                  'プログラムメモリとデータメモリが物理的に分離されている',
                isCorrect: true,
              },
              {
                content: 'キャッシュメモリを必ず搭載している',
                isCorrect: false,
              },
  ...
  try {
    ...
    res.json({
      ...
      debug_info: {
        ...
        database_url_prefix: databaseUrl
  ...
  try {
    ...
    for (let i = 0; i < migrations.length; i++) {
      try {
        ...
        results.push(
          `Migration ${i + 1}: ERROR - ${error instanceof Error ? error.message : 'Unknown error'}`,
        )
