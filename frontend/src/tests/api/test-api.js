  ...
  try {
    ...
    try {
      ...
      if (createResponse.ok) {
        ...
        const updateResponse = await fetch(`${API_BASE}/bookmarks/${createData.data.id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        const deleteResponse = await fetch(`${API_BASE}/bookmarks/${createData.data.id}`, {
          method: 'DELETE'
        })
        ...
        console.log('ℹ️ Create bookmark failed (expected if no questions):', errorData)
      }
    } catch (error) {
      console.log('ℹ️ Bookmark CRUD tests skipped (no valid questions):', error.message)
    }
    
    // Test 6: Check bookmark status
    console.log('\n6. Testing bookmark status check...')
    try {
      const statusResponse = await fetch(`${API_BASE}/bookmarks/question/test_question_1`)
