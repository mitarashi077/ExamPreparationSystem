                ...
                <Card>
                  <CardContent>
                    ...
                    <Typography variant="body2" sx={{ mb: 2 }}>
                      推定時間: {schedule.recommendations.estimatedTimeMinutes}
                      分
                    </Typography>
                  ...
                  <CardContent>
                    ...
                    <List dense>
                      <ListItem>
                        <ListItemIcon>
                          <Badge
                            badgeContent={schedule.schedule.today}
                            color="primary"
                          >
                            <AssignmentIcon />
                          </Badge>
                      ...
                      <ListItem>
                        <ListItemIcon>
                          <Badge
                            badgeContent={schedule.schedule.tomorrow}
                            color="secondary"
                          >
                            <AssignmentIcon />
                          </Badge>
                      ...
                      <ListItem>
                        <ListItemIcon>
                          <Badge
                            badgeContent={schedule.schedule.thisWeek}
                            color="info"
                          >
                            <AssignmentIcon />
                          </Badge>
                  ...
                  <CardContent>
                    ...
                    >
                      {[0, 1, 2, 3, 4, 5].map((level) => {
                        const count =
                          schedule.masteryDistribution[`level${level}`] || 0
                        return (
                          <Chip
                            key={level}
          ...
          <Card sx={{ mt: 2 }}>
            <CardContent>
              <Typography variant="body2" color="text.secondary">
                セッション統計: {sessionStats.correctItems}/
                {sessionStats.totalItems} 正解
                {sessionStats.totalItems > 0 && (
                  <span>
                    {' '}
                    (正答率:{' '}
                    {Math.round(
                      (sessionStats.correctItems / sessionStats.totalItems) *
                        100,
                    )}
                    %)
                  </span>
  ...
  const renderStats = () => (
    <Container maxWidth="lg">
      <Box sx={{ py: 3 }}>
        ...
        {/* 統計詳細は今後実装 */}
        <Alert severity="info">復習統計の詳細表示は今後実装予定です。</Alert>
      </Box>
    </Container>
  )
  ...
  return (
    <Box sx={{ minHeight: '100vh', backgroundColor: 'background.default' }}>
      ...
      <Dialog open={showResults} onClose={() => setShowResults(false)}>
        ...
        <DialogContent>
          <Typography gutterBottom>お疲れさまでした！</Typography>
          <Typography variant="body2" color="text.secondary">
          ...
          <Typography variant="body2" color="text.secondary">
            ...
            {sessionStats.totalItems > 0
              ? Math.round(
                  (sessionStats.correctItems / sessionStats.totalItems) * 100,
                )
              : 0}
            %
          </Typography>
